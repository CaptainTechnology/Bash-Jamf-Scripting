...
# ==============================================================================
# 1. THE `jamf` BINARY & MDM (Core Commands)
# ==============================================================================
...
dscl . -list /Users UniqueID | awk '$2 >= 501 {print $1}'  # List normal local users (even hidden)
...
# resetpassword                              # Works in recovery mode

sudo sysadminctl -deleteUser <username>            # Delete user (no home folder kept)
sudo sysadminctl -deleteUser <username> -keepHome  # Delete user, keep Home folder
sudo rm -rf /Users/<username>                      # Delete user home folder directly (bypasses permission errors)
...
sudo dsconfigad -remove -username admin -password password@123 # Remove system from AD domain

# ==============================================================================
# 3. SECURITY, FIREWALL & FILEVAULT
# ==============================================================================
...
sudo fdesetup changerecovery -personal       # Rotate personal recovery key (updates Jamf automatically)
...
# Kill processes
# sudo pkill jamf                                     # Kill the jamf binary
# sudo pkill -f "jamf policy"                         # Kill an already running jamf policy

tmutil status                                         # Time Machine Backup status
tmutil listbackups                                    # List available TM backups

# ==============================================================================
# 8. HANDY ONE-LINERS / EXTENSION ATTRIBUTES (EAs)
# ==============================================================================
# Change Screenshot location
# defaults write com.apple.screencapture location /Desktop/your_folder_Screenshot

# Get bundle ID by application name
# osascript -e 'id of app "Safari"' 

# Get bundle ID by application path (Don't add "com.microsoft.wdav" in smart group)
# mdls -name kMDItemCFBundleIdentifier "/Applications/Google Chrome.app" 

# Launch an app or file via terminal
# open -a safari https://www.google.com
# open -a safari image.png

# EA: Get logged-in user (safe for EAs, handles login window edge case)
# loggedInUser=$(scutil <<< "show State:/Users/ConsoleUser" | awk '/Name :/ { print $3 }')

# EA: Get current logged-in user's home directory
# userHome=$(dscl . -read /Users/"$loggedInUser" NFSHomeDirectory | awk '{print $2}')

# EA: Get free disk space in GB
# diskutil info / | awk -F'[()]' '/Free Space|Available Space/ {print $2}' | awk '{print $1}'

# EA: Check if a specific app is installed
# [ -d "/Applications/Google Chrome.app" ] && echo "Installed" || echo "Not Installed"

# EA: Get battery cycle count (for hardware EAs on laptops)
# system_profiler SPPowerDataType | grep "Cycle Count"

# EA: Check Apple Silicon vs Intel
# arch=$(uname -m)   # returns "arm64" or "x86_64"

# Tips for Jamf-specific use:
# - Always test scripts with `sudo` locally before pushing as a Jamf policy.
# - Wrap EA output in `<result>...</result>` tags.
# - Use `log show`/`log stream` with predicates for MDM profile push failures.
